// backend/src/services/payment_service.js
...
class PaymentService {
  ...
  async handle_callback(method, callback_data) {
    ...
    const payment = await Payment.findOneAndUpdate(
      { transaction_id: callback_result.transaction_id },
      {
        ...
        completed_at:
          callback_result.status === PAYMENT_STATUS.COMPLETED
            ? new Date()
            : null,
      },
      { new: true }
    ...
    if (callback_result.status === PAYMENT_STATUS.COMPLETED) {
    ...
    if (callback_result.status === PAYMENT_STATUS.FAILED) {
      await Order.findByIdAndUpdate(payment.order, {
  ...
  async check_payment_status(method, transaction_id) {
    ...
    switch (method) {
      case PAYMENT_METHOD.PAYME: {
        const result =
          await payme_service.get_payment_status(transaction_id);
    ...
    const payment = await Payment.findOneAndUpdate(
      { transaction_id },
      {
        status,
        completed_at:
          status === PAYMENT_STATUS.COMPLETED ? new Date() : null,
      },
      { new: true }
    );
  ...
  async get_user_payments(user_id) {
    const payments = await Payment.find({ user: user_id }).sort({
      createdAt: -1,
    });
  ...
  async refund_payment(payment_id) {
    ...
    switch (payment.method) {
      case PAYMENT_METHOD.PAYME:
        refund_data = await payme_service.refund_payment(
      ...
      case PAYMENT_METHOD.UZUM:
        refund_data = {
          refund_id: payment.transaction_id,
          status: "initiated",
        };
        break;
